...
def main():
    ...
    if fluid == "Liquid":
        if 1 < velocity < 10:
            note = "✅ Velocity is in the typical range for liquid lines."
        elif velocity >= 10:
            note = "⚠️ High velocity! May cause erosion or noise."
        else:
            note = "⚠️ Very low velocity. Risk of settling in solids/slurries."
    else:  # Gas
        if 10 < velocity < 40:
            note = "✅ Velocity is within common range for gas lines."
        else:
            note = "⚠️ Check for choking or noise. Use compressibility corrections if needed."
